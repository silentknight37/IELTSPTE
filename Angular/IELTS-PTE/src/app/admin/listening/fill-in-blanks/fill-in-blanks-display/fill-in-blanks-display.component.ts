...
@Component({
  selector: 'app-fill-in-blanks-display',
  templateUrl: './fill-in-blanks-display.component.html',
  styleUrls: ['./fill-in-blanks-display.component.scss']
})
export class FillInBlanksDisplayComponent implements OnInit {
  ...
  correctAnswerDisplay:string;
    ...
    ngOnInit() {
      this.spinner.show();
      this.commonService.getByAction('api/FillInBlanks').subscribe(res=>{
  ...
  checkAnswers(){
    ...
    this.AnswerList.forEach(element => {
     if(element.correctAnswer==element.selectAnswer){
       document.getElementById("blank-text-box-"+element.id).style.borderColor="green";
     }
     else{
       document.getElementById("blank-text-box-"+element.id).style.borderColor="red";
     }
    });
   }
