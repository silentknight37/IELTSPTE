...
@Component({
  selector: 'app-summarise-writte-text-display',
  templateUrl: './summarise-writte-text-display.component.html',
  styleUrls: ['./summarise-writte-text-display.component.scss']
})
export class SummariseWritteTextDisplayComponent implements OnInit {
  ...
  reviewEmail=new ReviewEmail();
    ...
    ngOnInit() {
      this.spinner.show();
      this.commonService.getByAction('api/WritingSummariseWrittenText').subscribe(res=>{
  ...
  sendReview(){
    ...
    this.questionSetupService.createwithAction(this.reviewEmail,"api/WritingSummariseWrittenText/SendReview").subscribe(res=>{
      this.response = res;   
    },
  error =>{
